...
public class Job15 {
    ...
    public static class JobMapper extends Mapper<LongWritable, Text, Text, FloatWritable> {
        ...
        private FloatWritable fertilityRate = new FloatWritable();
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if(fields.length>3){
                ...
                if(!"Value".equals(fields[3])){
                    ...
                    float fertility = Float.parseFloat(fields[3]);
                    total += fertility;
                    String decade = "2000s";
                    if(year<2000) decade= "1990s";
                    if(year>2009) decade = "2010s";
                    country.set(countryName+","+decade+","+fields[2]);
                    fertilityRate.set(fertility);
                    context.write(country,fertilityRate);
                }
    ...
    public static class CustomTextOutputFormat extends TextOutputFormat<Text, FloatWritable> {
        private static final String HEADER = "Country,decade,quintile,Average";
