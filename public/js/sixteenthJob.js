...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-sixteenth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-sixteenth-job-data');
...
chart.selectAll(".bar")
  ...
  function fetchCountries() {
    return fetch('/api/get-countries-sixteenth-job')
...
function fetchdecade() {
    return fetch('/api/get-decade-sixteenth-job')
      .then(response => response.json())
      .then(decadeData => {
