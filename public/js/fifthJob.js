...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-fifth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-fifth-job-data');
...
function updateVisualization(data, selectedYear, selectedregion, selectedSex) {
    var filteredData;

    if (selectedYear === "All Years" && selectedregion === "All regions" && selectedSex == "All Sexes") {
...
yearSelect.addEventListener("change", async function (event) {
    ...
    updateVisualization(data, selectedYear, selectedregion, selectedSex);
});
...
sexSelect.addEventListener("change", async function (event) {
    ...
    // Update the visualization based on the selected year, region, and sex
    updateVisualization(data, selectedYear, selectedregion, selectedSex);
});

// Fetch years data and return a promise
function fetchYears() {
    return fetch('/api/get-years-fifth-job')
...
function fetchSexes() {
    return fetch('/api/get-sex-fifth-job')
        .then(response => response.json())
        .then(sexes => {
...
function fetchregions() {
    return fetch('/api/get-regions-fifth-job')
      .then(response => response.json())
      .then(regions => {
