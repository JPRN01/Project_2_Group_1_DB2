...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-twentieth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-twentieth-job-data');
...
chart.selectAll(".bar")
  ...
  function fetchCountries() {
    return fetch('/api/get-countries-twentieth-job')
...
function fetchdecade() {
    return fetch('/api/get-decade-twentieth-job')
      .then(response => response.json())
      .then(decadeData => {
