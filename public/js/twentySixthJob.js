...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-twentySixth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-twentySixth-job-data');
...
var svgWidth = 800;
var svgHeight = 400;
var margin = { top: 200, right: 100, bottom: 95, left: 120 };
...
function updateVisualization(data, selectedsubregion, selectedyear) {
  filteredData = [];
  ...
  filteredData.forEach((d) => {
      ...
      aggregates[subregion].total += parseFloat(d.total);
...
chart.selectAll(".bar")
  ...
  subregionSelect.addEventListener("change", async function (event) {
    ...
    updateVisualization(data, selectedsubregion, selectedyear);
  });
  ...
  function fetchsubregions() {
    return fetch('/api/get-subregions-twentySixth-job')
...
function fetchyear() {
    return fetch('/api/get-year-twentySixth-job')
      .then(response => response.json())
      .then(yearData => {
