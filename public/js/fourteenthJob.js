...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-fourteenth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-fourteenth-job-data');
...
chart.selectAll(".bar")
  ...
  function fetchCountries() {
    return fetch('/api/get-countries-fourteenth-job')
...
function fetchdecade() {
    return fetch('/api/get-decade-fourteenth-job')
      .then(response => response.json())
      .then(decadeData => {
