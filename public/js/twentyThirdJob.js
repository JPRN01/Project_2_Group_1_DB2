...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-twentyThird-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-twentyThird-job-data');
...
chart.selectAll(".bar")
  ...
  function fetchCountries() {
    return fetch('/api/get-countries-twentyThird-job')
...
function fetchdecade() {
    return fetch('/api/get-decade-twentyThird-job')
      .then(response => response.json())
      .then(decadeData => {
