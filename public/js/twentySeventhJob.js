...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-twentySeventh-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-twentySeventh-job-data');
...
function updateVisualization(data, selectedregion, selectedyear, selectedsex) {
  filteredData = [];
  ...
  filteredData.forEach((d) => {
      ...
      aggregates[region].total += d.total;
      if (!aggregates[region].years.includes(d.year)) {
          aggregates[region].years.push(d.year);
      }
      if (!aggregates[region].sexes.includes(d.sex)) {
        aggregates[region].sexes.push(d.sex);
    }
  });
...
chart.selectAll(".bar")
  ...
  regionSelect.addEventListener("change", async function (event) {
    ...
    updateVisualization(data, selectedregion, selectedyear, selectedsex);
  });
  ...
  function fetchregions() {
    return fetch('/api/get-regions-twentySeventh-job')
...
function fetchyear() {
    return fetch('/api/get-year-twentySeventh-job')
      .then(response => response.json())
      .then(yearData => {
  ...
  // Fetch year data and return a promise
function fetchsex() {
    return fetch('/api/get-sex-twentySeventh-job')
      .then(response => response.json())
      .then(sexData => {
