...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-nineteenth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-nineteenth-job-data');
...
chart.selectAll(".bar")
  ...
  function fetchCountries() {
    return fetch('/api/get-countries-nineteenth-job')
...
function fetchdecade() {
    return fetch('/api/get-decade-nineteenth-job')
      .then(response => response.json())
      .then(decadeData => {
