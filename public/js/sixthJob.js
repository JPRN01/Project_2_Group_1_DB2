...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-sixth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-sixth-job-data');
...
function updateVisualization(data, selectedYear, selectedsubRegion, selectedSex) {
    var filteredData;

    if (selectedYear === "All Years" && selectedsubRegion === "All SubRegions" && selectedSex == "All Sexes") {
...
yearSelect.addEventListener("change", async function (event) {
    ...
    updateVisualization(data, selectedYear, selectedsubRegion, selectedSex);
});
...
sexSelect.addEventListener("change", async function (event) {
    ...
    // Update the visualization based on the selected year, subRegion, and sex
    updateVisualization(data, selectedYear, selectedsubRegion, selectedSex);
});

// Fetch years data and return a promise
function fetchYears() {
    return fetch('/api/get-years-sixth-job')
...
function fetchSexes() {
    return fetch('/api/get-sex-sixth-job')
        .then(response => response.json())
        .then(sexes => {
...
function fetchsubRegions() {
    return fetch('/api/get-subRegions-sixth-job')
      .then(response => response.json())
      .then(subRegions => {
