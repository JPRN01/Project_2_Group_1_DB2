...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-eighteenth-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-eighteenth-job-data');
...
function updateVisualization(data, selectedCountry, selecteddecade) {
  ...
  // Calculate the percentage for the filtered data
  const aggregates = {};
  ...
  filteredData.forEach((d) => {
      ...
      aggregates[country].percentage += parseFloat(d.percentage);
...
var maxHomicidepercentage = d3.max(filteredData, function (d) { return parseFloat(d.percentage); });

// Update the scales' domains with the filtered data
xScale.domain(filteredData.map(function (d) {
  return d.country;
}));
yScale.domain([0, maxHomicidepercentage]);
...
chart.selectAll(".bar")
    ...
    .attr("fill", "#6A5ACD")
    .attr("height", function (d) { return chartHeight - yScale(d.percentage); })
  ...
  function fetchCountries() {
    return fetch('/api/get-countries-eighteenth-job')
...
function fetchdecade() {
    return fetch('/api/get-decade-eighteenth-job')
      .then(response => response.json())
      .then(decadeData => {
