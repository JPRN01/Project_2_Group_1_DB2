document.getElementById('uploadButton').addEventListener('click', function() {
  fetch('/api/upload-first-job-csv', {
    method: 'POST'
  })
...
async function fetchData() {
  try {
      const response = await fetch('/api/get-first-job-data');
...
var yScale = d3.scaleLinear()
  ...
  function updateVisualization(data, selectedCountry) {
    ...
    var maxHomicideAverage = d3.max(filteredData, function (d) {
      return d.average;
    });
...
function fetchCountries() {
  return fetch('/api/get-countries-first-job')
