...
document.getElementById('uploadButton').addEventListener('click', function() {
    fetch('/api/upload-twentySecond-job-csv', {
      method: 'POST'
    })
...
async function fetchData() {
    try {
        const response = await fetch('/api/get-twentySecond-job-data');
...
var svgWidth = 1900;
var svgHeight = 500;
var margin = { top: 130, right: 20, bottom: 120, left: 80 };
...
chart.selectAll(".bar")
  ...
  function fetchCountries() {
    return fetch('/api/get-countries-twentySecond-job')
...
function fetchdecade() {
    return fetch('/api/get-decade-twentySecond-job')
      .then(response => response.json())
      .then(decadeData => {
